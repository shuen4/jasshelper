...
type
  ...
  private
    ...
    FActualCharacterSetTable : Array of Set of AnsiChar;
    FActualCharacterSetTableAllCaps : Array of Set of AnsiChar;
...
begin
  ...
  for i:=0 to FCharacterSetTable.count-1 do begin
      FActualCharacterSetTable[i]:=[];
      FActualCharacterSetTableAllCaps[i]:=[];
      s:=UpperCase(FCharacterSetTable.strings[i]);
      for j:=1 to Length(FCharacterSetTable.strings[i]) do begin
          Include(FActualCharacterSetTableAllCaps[i],s[j]);
          Include(FActualCharacterSetTable[i],AnsiChar(FCharacterSetTable.strings[i][j]));
...
begin
   ...
   Start := TToken.Create;
   //<vexorian> assign LineNumber
   Start.LineNumber:=CurrentLineNumber;
...
begin
  ...
  if not FSource.Done then begin
    while not Done do begin
      ...
      if not FSource.ReadCharFromBuffer(CurrentPosition,ch) then Found := False
      else begin
        n := 0;
        Found := False;
        ...
        while (n < DFA[CurrentDFA].EdgeCount) and (not Found) do begin
          ...
          if FCompareMode = vbTextCompare then begin
               ...
               inset:=UpCase(ch) in FActualCharacterSetTableAllCaps[CharSetIndex];
          end else begin
               inset:=ch in FActualCharacterSetTable[CharSetIndex];
          end;
     ...
     case ActionTables[FCurrentLALR][Index].Action of
       ...
       ActionReduce : begin
         ...
         if (FTrimReductions) and (CurrentRule.ContainsOneNonTerminal) then begin
           ...
           Head := TToken.Create;
           //<vexorian> assign LineNumber
           Head.LineNumber:=NextToken.LineNumber;
